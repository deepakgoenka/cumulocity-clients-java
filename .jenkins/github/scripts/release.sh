...
#if it is a release on githubdev branch, hg branch (git symbolic-ref --short HEAD) will return release/rX.X.X as it is the branch created in previous step.
# If it is a release/hotfix on release branch it should just push the branch it was on
branch_name=$(git symbolic-ref --short HEAD)
if [ "githubdev" == "${branch_name}" ]; then
